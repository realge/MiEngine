...
VkDescriptorSetLayout IBLSystem::createDescriptorSetLayout() {
    ...
    // Create bindings for IBL textures (3 cubemaps + 1 2D texture)
    ...
    // Binding 2: Prefiltered environment cubemap
...
std::vector<VkDescriptorSet> IBLSystem::createDescriptorSets(VkDescriptorPool descriptorPool, uint32_t frameCount) {
    ...
    if (!environmentMap || !irradianceMap || !prefilterMap || !brdfLUT) {
    ...
    for (uint32_t i = 0; i < frameCount; i++) {
        ...
        imageInfos[0].imageView = environmentMap->getImageView();
        imageInfos[0].sampler = environmentMap->getSampler();
        
        // Irradiance map
        imageInfos[1].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        imageInfos[1].imageView = irradianceMap->getImageView();
        imageInfos[1].sampler = irradianceMap->getSampler();
        
        // Prefiltered environment map
        imageInfos[2].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        imageInfos[2].imageView = prefilterMap->getImageView();
        imageInfos[2].sampler = prefilterMap->getSampler();
        
        // BRDF LUT
        imageInfos[3].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        imageInfos[3].imageView = brdfLUT->getImageView();
        imageInfos[3].sampler = brdfLUT->getSampler();
        ...
        descriptorWrites[3].dstSet = sets[i];
